...
class NewVisitorTest(LiveServerTestCase):
    ...
    def wait_for_row_in_list_table(self, row_text):
        '''лжидать строку в таблице списка'''
    ...
    def test_can_start_a_list_for_one_user(self):
        '''тест: можно начать список для нового пользователя'''
        # Эдит слышал про крутое новое онлайн-приложение со списком неотложных дел.
        # Она решает оценить его домашнюю страницу
        self.browser.get(self.live_server_url)

        # Она видит, что заголовок и шапка программы говорят о списках неотложных дел
        ...
        # Она вводит "Сделать мушку их павлиньих перьев" (Эдит очень методична)
        inputbox = self.browser.find_element_by_id('id_new_item')
        inputbox.send_keys('Сделать мушку их павлиньих перьев')
        inputbox.send_keys(Keys.ENTER)

        # Страница снова обновляется, и теперь показывает оба элемента ее списка
        self.wait_for_row_in_list_table('1: Купить павлиньи перья')
        self.wait_for_row_in_list_table('2: Сделать мушку их павлиньих перьев')
    ...
    def test_multiple_users_can_start_lists_at_different_urls(self):
        ...
        # Френсис начинает новый список, вводя свой новый элемент. Он менее интересен, чем список Эдит...
        inputbox = self.browser.find_element_by_id('id_new_item')
        ...
        # Опять-таки нет следов списка Эдит
        page_text = self.browser.find_element_by_tag_name('body').text
        self.assertNotIn('Купить павлиньи перья', page_text)
